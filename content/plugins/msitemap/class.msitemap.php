...
class msitemap {
	...
	function __construct() {
		...
		$this->path = EMLOG_ROOT . '/m/' . $sitemap_name;
	...
	private function setData() {
		...
		$data[] = array('url'=>$logUrl,'lastmod'=>time(),'changefreq'=>'always','priority'=>'1.0');
		//日志
		...
		while($row = $this->db->fetch_array($query)) {
			...
			$data[] = array('url'=>$this->geturl($row['gid']),'lastmod'=>$lastmod,'changefreq'=>$this->changefreq[0],'priority'=>$this->priority[0]);
		}
	...
	function geturl($blogId){
		$logUrl=str_replace("www", "m",BLOG_URL);
		$logUrl = $logUrl . 'post-' . $blogId . '.html';
		return $logUrl;
	}
