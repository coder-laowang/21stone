...
?>
<div class=containertitle><b>MSiteMap</b>
...
<div class=line></div>
<form action="plugin.php?plugin=msitemap&action=setting" method="post">
<div>
	<p>Msitemap文件名：
...
<form action="plugin.php?plugin=msitemap&action=setting" method="post">
	...
	<p><input type="submit" value="更新Msitemap" class="submit" /></p>
</div>
</form>
<script>
$("#msitemap").addClass('sidebarsubmenu1');
...
function plugin_setting()
{
	...
	if(!isset($_POST['update'])) {
		...
		if($sitemap_name2 != '' && $sitemap_name != $sitemap_name2) {
			if(!@rename(EMLOG_ROOT . '/m/' . $sitemap_name, EMLOG_ROOT . '/m/' . $sitemap_name2)) {
				emMsg("重命名文件{$sitemap_name}失败,请设置根目录下{$sitemap_name}权限为777（LINUX)/everyone可写（windows）",'./plugin.php?plugin=msitemap');
			}
		...
		if(!@file_put_contents(EMLOG_ROOT . '/content/plugins/msitemap/config',serialize(compact('sitemap_name','sitemap_changefreq','sitemap_priority','sitemap_show_footer','sitemap_comment_time')))) {
			emMsg("更新配置失败,请设置文件content/plugins/msitemap/config权限为777（LINUX）/everyone可写（windows）",'./plugin.php?plugin=msitemap');
		}
	}
	if(!msitemap_update()) {
		emMsg("更新msitemap失败,请设置根目录下{$sitemap_name}权限为777（LINUX）/everyone可写（windows）",'./plugin.php?plugin=msitemap');
	}
	return true;
}