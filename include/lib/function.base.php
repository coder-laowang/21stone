 ...
 * @param string $url 页码的地址
 */
function pagination_m($count, $perlogs, $page, $url, $anchor = '') {
	$pnums = @ceil($count / $perlogs);
	$re = '';
	//$urlHome = preg_replace("|[\?&/][^\./\?&=]*page[=/\-]|", "", $url);
	$urlHome=$url.'1';
	for ($i = $page - 5; $i <= $page + 5 && $i <= $pnums; $i++) {
		if ($i > 0) {
			if ($i == $page) {
				...
				$re .= " <a href=\"$urlHome$anchor.html\" target=\"_blank\">$i</a> ";
				$re .= " <a href=\"$urlHome$anchor.html\" target=\"_blank\">$i</a> ";
			} else {
				$re .= " <a href=\"$url$i$anchor.html\" target=\"_blank\">$i</a> ";
			}
		}
	}
	if ($page > 6)
		$re = "<a href=\"{$urlHome}$anchor.html\"  target=\"_blank\" title=\"首页\">&laquo;</a><em>...</em>$re";
		if ($page + 5 < $pnums)
			$re .= "<em>...</em> <a href=\"$url$pnums$anchor.html\"  target=\"_blank\" title=\"尾页\">&raquo;</a>";
			if ($pnums <= 1)
				$re = '';
				return $re;
}
